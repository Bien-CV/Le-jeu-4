...
#define H_HEADER
		...
		typedef struct { char name[MaxString] ; int range ; t_type type ; int damage_coeff ;} t_skill;
		typedef struct { t_skill a; t_skill b; t_skill c; t_skill d; t_skill e; t_skill wait; } t_skill_list;
		typedef struct{char name[MaxString] ; t_coord position; t_orientation orientation ; t_status status ; t_stats stats ; t_skill_list skill; t_camp camp; int nbActionTour;} t_character ;
